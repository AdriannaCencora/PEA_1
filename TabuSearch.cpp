...
void TabuSearch::displayRouteDetails() {
...
//              std::to_string(graph->getNumberOfCities()) + ")\n";
...
void TabuSearch::setDefaultParameters() {
    cadency = graph->getNumberOfCities() / 2;
...
void TabuSearch::generateStartRoute() {
    ...
    for (int i = 0; i < numberOfCities - 1; ++i) {
        ...
        for (int j = 0; j < numberOfCities; ++j) {
            ...
            distancesFromCurrentVertex.emplace_back(std::make_pair(graph->getDistance(currentVertex, j), j));
        }
...
int TabuSearch::calculateRouteDistance(std::vector<int> &route) {
    ...
    for (int i = 0; i < route.size(); ++i) {
        ...
        distance += graph->getDistance(currentVertex, nextVertex);
    }

    return distance;
}

std::vector<int> TabuSearch::generateRandomRoute() {
    std::vector<bool> visitedVertices;
